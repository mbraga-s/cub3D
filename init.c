...
/*   Updated: 2024/09/14 18:58:45 by manumart         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	map->no.img = mlx_xpm_file_to_image(cub3d->mlx, map->n_texture, &map->no.w,
			&map->no.h);
	if (!map->no.img)
		error_end_game(cub3d, "Error loading NO textures.\n");
	map->no.addr = mlx_get_data_addr(map->no.img, &map->no.bpp, &map->no.l_lgt,
			&map->no.endian);
	map->so.img = mlx_xpm_file_to_image(cub3d->mlx, map->s_texture, &map->so.w,
			&map->so.h);
	if (!map->so.img)
		error_end_game(cub3d, "Error loading SO textures.\n");
	map->so.addr = mlx_get_data_addr(map->so.img, &map->so.bpp, &map->so.l_lgt,
			&map->so.endian);
	map->we.img = mlx_xpm_file_to_image(cub3d->mlx, map->w_texture, &map->we.w,
			&map->we.h);
	if (!map->we.img)
		error_end_game(cub3d, "Error loading WE textures.\n");
	map->we.addr = mlx_get_data_addr(map->we.img, &map->we.bpp, &map->we.l_lgt,
			&map->we.endian);
	map->ea.img = mlx_xpm_file_to_image(cub3d->mlx, map->e_texture, &map->ea.w,
			&map->ea.h);
	if (!map->ea.img)
		error_end_game(cub3d, "Error loading EA textures.\n");
	map->ea.addr = mlx_get_data_addr(map->ea.img, &map->ea.bpp, &map->ea.l_lgt,
			&map->ea.endian);
}
